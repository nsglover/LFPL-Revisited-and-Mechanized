\begin{abstract}
  LFPL, by Hofmann (1999), uses an affine type system to characterize the functions on natural numbers computable in polynomial time.
  %
  ...
  %
  % TODO: Make macros for names (LFPL, LFPL+, AARA, etc).
  The soundness proof works with a language $\text{LFPL}^+$, which extends LFPL with several powerful language features, thereby providing a stronger theorem than pre-existing results.
  %
  The proof is novel, using a technique by Aehlig and Schwichtenberg (2002) to construct explicit polynomials that bound the cost of an $\text{LFPL}^+$ expression with respect to a big-step cost semantics.
  %
