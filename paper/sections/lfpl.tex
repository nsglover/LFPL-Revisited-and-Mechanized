...
\begin{mathpar}
\Rule{ExpI}{\types{!\ctx}{\code{wrap}(M)}{\; !A}}{\types{!\ctx}{M}{A}}
\and
\Rule{ExpE}{\types{\ctx}{\code{unwrap}(M)}{A}}{\types{\ctx}{M}{\; !A}}
\and
\Rule{Contr}{\types{\ctxcons{\ctx}{x}{\; !A}}{[x/y]M}{B}}{\types{\ctxcons{\ctxcons{\ctx}{x}{\; !A}}{y}{\; !A}}{M}{B}}
\end{mathpar}
...
\begin{lstlisting}
expCalls : L(1) -> !(L(1) -> L(1)) -> !(L(1) -> L(1))
expCalls nil f = f
expCalls (cons (_, _, l)) =
  let r = expCalls l f in
  wrap (lam x . unwrap(r) (unwrap(r) x))
\end{lstlisting}
Calling $\tmapp{\tmapp{\code{expCalls}}{\ell}}{f}$ yields a function which calls $f$ on itself $2^{|\ell|}$ times. If $f$ incurs any cost, then the resulting function will definitely not run in polynomial time.
