...
| cons (d, x, r) => letp (f, m) = r in
  (lam n . case (lunfold n) .
    ...
    , cons (d, <>, m))
\end{lstlisting}
This example is very important for the completeness proof, so it is worth stating its semantics: If $x \in \tpsem{A}^*$, then $\tmsem{susp}{\envnil}(x) = (f, m)$, where $f(m) = x$. Note that $m$ is the unique element of $\tpsem{\tpunit}^*$ with length $|m|$, so we recover $x$ as long as we give $f$ enough diamonds.
\end{example}
